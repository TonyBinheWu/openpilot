...
def executable_problem(path, updater=False):
  try:
  ...
  if updater and header.startswith(b"#!/usr/bin/env python3\n"):
    try:
      with zipfile.ZipFile(path) as archive:
        if "__main__.py" not in archive.namelist():
          return f"Updater zipapp is missing __main__.py: {path}"
        bad = archive.testzip()
        if bad:
          return f"Updater zipapp CRC check failed: {bad}"
      return None
    except (OSError, zipfile.BadZipFile, RuntimeError) as error:
      return f"Invalid/truncated updater zipapp: {path}: {error}"
  if updater and header[:4] != b"\x7fELF":
    return f"Invalid updater: expected upstream Python zipapp or ELF binary: {path}"
  if updater and (len(header) < 20 or header[4:6] != b"\x02\x01" or int.from_bytes(header[18:20], "little") != 183):
...
def main():
  ...
  if args.check_updater:
    problem = executable_problem(args.check_updater, updater=True)
    if problem:
  ...
  try:
    ...
    print("[hkg-boot] Stage storage unavailable; continuing with the launcher", flush=True)
    log.close()
    os.execv("/bin/bash", ["bash", str(root / "launch_chffrplus.sh")])
