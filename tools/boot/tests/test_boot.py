...
class DiagnosticTests(unittest.TestCase):
  ...
  def test_missing_binary(self):
    self.assertIn("Missing", boot.executable_problem(self.root / "missing", updater=True))
  ...
  def test_invalid_elf(self):
    self.assertIn("ELF", boot.executable_problem(self.binary(b"not a binary"), updater=True))

  def test_arm64_elf(self):
    elf = bytearray(64)
    elf[:6] = b"\x7fELF\x02\x01"
    elf[18:20] = (183).to_bytes(2, "little")
    self.assertIsNone(boot.executable_problem(self.binary(elf), updater=True))

  def test_wrong_architecture(self):
    elf = bytearray(64)
    elf[:6] = b"\x7fELF\x02\x01"
    elf[18:20] = (62).to_bytes(2, "little")
    self.assertIn("AArch64", boot.executable_problem(self.binary(elf), updater=True))

  def test_python_zipapp(self):
    path = self.binary(b"#!/usr/bin/env python3\n")
    with zipfile.ZipFile(path, "a") as archive:
      archive.writestr("__main__.py", "print('test')\n")
    self.assertIsNone(boot.executable_problem(path, updater=True))

  def test_zipapp_missing_entry_point(self):
    path = self.binary(b"#!/usr/bin/env python3\n")
    with zipfile.ZipFile(path, "a") as archive:
      archive.writestr("other.py", "pass\n")
    self.assertIn("__main__.py", boot.executable_problem(path, updater=True))

  def test_truncated_zipapp(self):
    path = self.binary(b"#!/usr/bin/env python3\nPK\x03\x04broken")
    self.assertIn("truncated", boot.executable_problem(path, updater=True))
