...
- 需要 OS 更新時先確認 updater 不是 LFS pointer，具有執行權限且是 AArch64 ELF；實際下載／刷寫／驗證仍交給同一套 sunnypilot updater 與 agnos.py。
...
包含 updater 故障只執行一次、OS 不匹配不得啟動 build、版本相同跳過更新、build 失敗不得啟動 manager、ELF/LFS/權限檢查、日誌輪替與磁碟錯誤隔離。
